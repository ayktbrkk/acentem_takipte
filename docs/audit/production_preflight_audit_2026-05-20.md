...
İlk audit pasif/statik inceleme, dependency sorguları, test/build komutları, Bench test denemesi ve local/prod üzerinde pasif `HEAD/GET` kontrolleriyle yürütüldü. Canlı sistemde exploit, veri değiştirme, brute force, destructive işlem veya aktif saldırı testi yapılmadı.
