...
## 2026-03-11 Guncel Kapanis

- **Aktif dalga:** Dalga 7
- **Aktif faz:** yok (kapanis)
- **Son tamamlanan faz:** Faz 372
- **Durum:** Faz 16 e2e/session/report/scheduled erisim gate smoke dogrulamasi tamamlandi; Faz 17 admin job snapshot erisim testi, role-gated e2e ve GET/POST kontrat dogrulamasi eklendi; Faz 17 kapanis plan/roadmap/README ile senkronlandi. Faz 18 icinde dashboard read endpoint method guard, frontend GET hizalamasi, workbench aggregate count iyilestirmesi ve benchmark threshold/output hardening tamamlandi. Faz 19 icinde scenario bazli kosum filtresi, benchmark preset JSON dosyalari, iki dilli kullanim notu, `--preset default` ve standart artefact output destegi eklendi. Faz 20 ilk diliminde `--list-presets` ve JSON rapor meta standardi eklendi. Faz 21 ilk diliminde Markdown benchmark artefact ozeti eklendi. Faz 22 ilk diliminde preset threshold baseline standardi eklendi. Faz 23 ilk diliminde benchmark preset katalogu `quick/default/full` olarak genisletildi. Faz 24 ilk diliminde preset kesfi threshold ve aciklama ozetiyle zenginlestirildi. Faz 25 preset discoverability JSON katalog cikisi ile kapatildi. Faz 26 ownership assignment lifecycle aksiyonlari CustomerDetail, PolicyDetail, CommunicationCenter ve operator yuzeylerinde hizalanarak kapatildi. Faz 27 aux/list-detail operator action parity notification draft/outbox, task, reminder ve ownership assignment yuzeylerinde kapatildi. Faz 28 operator yuzeyleri arasi baglamsal gecis parity reminder/task/ownership_assignment/draft/outbox kapsami ile kapatildi. Faz 29 ilk diliminde CommunicationCenter icine `return_to` baglamli geri donus aksiyonu ve test kontrati eklendi; ikinci dilimde AuxWorkbench/AuxRecordDetail gecislerine return_to aktarimi tamamlandi; ucuncu dilimde CustomerDetail/LeadDetail/OfferDetail gecislerine return_to aktarildi; dorduncu dilimde CommunicationCenter toolbar geri donus butonu eklendi; besinci dilimde return_to butonu context disinda da testle sabitlendi; altinci dilimde return_to yokken router back fallback davranisi eklendi; yedinci dilimde return_to guvenli path kontrolu ve unsafe URL fallback testi eklendi; sekizinci dilimde resolveSameOriginPath tabanli same-origin URL cozumleme eklendi; dokuzuncu dilimde ClaimsBoard notification gecisinde return_to parametresi ve sayfa testi eklendi; onuncu dilimde Dashboard follow-up CommunicationCenter gecisine return_to parametresi eklendi; Faz 29 kapanis senkronu tamamlandi. Faz 30 ilk diliminde Reports view-state (kolon gorunurlugu) route senkronu ve geri yazimi testleri eklendi; ikinci dilimde scheduled reports run/save/remove ve export PDF aksiyonlari test kapsami ile genisletildi; ucuncu dilimde export XLSX ve export failure fallback test kapsami eklendi; Faz 30 kapanis notu tamamlandi. Faz 31 ilk diliminde scheduled reports run/save/remove hata fallback test kapsami eklendi; ikinci dilimde scheduled reports load failure ve report load error test kapsami eklendi; Faz 31 kapanis notu tamamlandi. Faz 32 ilk diliminde payment status raporunda branch bazli policy scope filtresi ve test kapsami eklendi; ikinci dilimde communication operations raporunda branch filtresi eklendi ve test kapsami guncellendi; ucuncu dilimde reconciliation operations raporunda branch filtresi eklendi ve test kapsami guncellendi ve kapatildi. Faz 33 ilk diliminde reports runtime export format normalize ve scheduled summary error fallback testleri eklendi ve kapatildi. Faz 34 ilk diliminde export format `xls` alias destegi ve test kapsami eklendi ve kapatildi. Faz 35 ilk diliminde export popup blocked fallback davranisi ve test kapsami eklendi ve kapatildi. Faz 36 ilk diliminde reports runtime download response None-safe kapsami eklendi ve kapatildi. Faz 37 ilk diliminde build_safe_report_payload hata log test kapsami eklendi ve kapatildi. Faz 38 ilk diliminde export format `xlsm/excel` alias destegi ve test kapsami eklendi ve kapatildi. Faz 39 ilk diliminde download response filters None-safe kapsami eklendi ve kapatildi. Faz 40 ilk diliminde Reports export URL office_branch filtresi testle sabitlendi ve kapatildi. Faz 41 ilk diliminde export normalize MIME aliaslari ve `xlsb` kapsami ile genisletildi; ikinci dilimde tabular export title fallback tam locale -> base locale -> `en` sirasi ile sertlestirildi; ucuncu dilimde reporting filter normalize akisi `null`/`none` stringlerini dislayacak sekilde guncellendi; dorduncu dilimde payment report branch-policy lookup ayni request icinde cache'lenir hale getirildi ve test kapsami ile kapatildi. Faz 42 ilk diliminde report title katalogu communication/reconciliation/claims operations raporlarini kapsayacak sekilde genisletildi; ikinci dilimde `build_report_title(...)` locale fallback zinciri tam locale -> base locale -> `en` sirasi ile sertlestirildi; ucuncu dilimde scheduled report config summary/normalize/dispatch format akisi runtime export normalize helper'i ile tek kaynak haline getirildi ve test kapsami ile kapatildi. Faz 43 ilk diliminde list export screen/tabular query parse akisi invalid JSON icin bos payload fallback'i ile sertlestirildi; ikinci dilimde list export label lokalizasyonu tam locale -> base locale -> `en` sirasi ile hizalandi; ucuncu dilimde tabular payload export akisi invalid JSON durumunda guvenli default payload ile calisir hale getirildi ve test kapsami ile kapatildi. Faz 44 ilk diliminde list export permission doctype normalize akisi comma-separated string girdiyi destekler hale getirildi; ikinci dilimde invalid permission doctype JSON fallback davranisi patlamadan calisacak sekilde sertlestirildi; ucuncu dilimde permission doctype listesi tekrar eden girdileri tekillestirir hale getirildi ve test kapsami ile kapatildi. Faz 45 / Issue: List Export Permission Contract Hardening icinde `export_tabular_payload(...)` en az bir permission doctype zorunlulugu ile sertlestirildi; bos izin listesi durumunda export akisi kontrollu hata verir hale getirildi ve test kapsami ile kapatildi. Faz 46 / Issue: Report Payload Limit Contract Hardening icinde `report_registry.build_report_payload(...)` limit normalize akisi `cint` tabanli hale getirildi; non-numeric ve string limit girdileri icin kontrat test ile sabitlendi. Faz 47-51 issue serisinde tabular export kolon inference, kolon tekillestirme, tabular filter shape hardening, screen query shape hardening ve row payload shape kontrolu tamamlandi. Faz 52-61 issue serisinde title normalize, kolon string input, JSON filter/query coercion, dashboard row payload guard, boolean token genislemesi ve dict serialize fallback tamamlandi. Faz 62-71 issue serisinde report download locale/title trim, download payload coercion, scheduled config parse hardening ve export key fallback kontratlari tamamlandi. Faz 72-81 issue serisinde report export filename/title trim, PDF/XLSX render input coercion ve workbook payload hardening tamamlandi.
- **Siradaki adim:** yeni issue secimi veya yeni uygulama dalgasi.

### Arsiv Notu
- Asagidaki uzun notlar tarihsel yol haritasi kaydidir.
- Guncel yon bilgisi icin sadece bu ust blok referans alinmalidir.
- Aktif odak kapanis durumuna cekildi; yeni issue secimi bekleniyor.
...
- 2026-03-11: Faz 16 manual kapanış turu tamamlandi; `frontend/tests/e2e/at-smoke.spec.js` dosyasıyla session context, policy report contract ve scheduled erişim gate doğrulaması eklendi.
