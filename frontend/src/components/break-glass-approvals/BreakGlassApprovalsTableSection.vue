<template>
  <SectionPanel :title="t('pendingTitle')" :count="pendingRows.length">
    ...
    <div v-else class="mt-4">
      ...
      >
        <template #cell(justification)="{ row }">
          <p class="line-clamp-3 text-[13px] font-semibold text-slate-900">{{ displayFallback(row.justification) }}</p>
          <p class="mt-1 text-xs text-slate-500">{{ displayFallback(row.reference) }}</p>
        </template>

        <template #cell(actions)="{ row }">
          <div class="min-w-[220px]" @click.stop>
...
const columns = computed(() => [
  {
    key: "name",
    label: props.t("request"),
    type: "stacked",
    secondaryKey: "user",
  },
  {
    key: "access_type",
    label: props.t("accessType"),
    format: (value) => props.mapAccessType(value),
  },
  {
    key: "created_at",
    label: props.t("createdAt"),
    format: (value) => displayFallback(value),
  },
  {
    key: "justification",
    label: props.t("justification"),
    cellClass: () => "max-w-[360px] align-top",
  },
  {
    key: "actions",
