<template>
  ...
  >
    <div class="mt-1 flex items-center justify-between gap-3 text-xs text-slate-500">
      <span>{{ columnsLabel }}: {{ columnsSummaryLabel }}</span>
      <span v-if="exportLoading">{{ exportingLabel }}</span>
    </div>
    ...
    <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50/70 p-3">
      <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-3">
        <p class="text-xs text-slate-600">{{ columnHint }}</p>
      ...
      <div class="mt-3 rounded-xl border border-slate-200 bg-white/90 p-3">
        <div class="flex items-center justify-between gap-2">
          <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">{{ columnsLabel }}</p>
    ...
    <template v-else>
      ...
      <div class="mt-4 overflow-hidden rounded-lg border border-gray-200">
        ...
        <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3">
          <p class="text-xs text-gray-400">{{ sortedRows.length }} / {{ sortedRows.length }} {{ showingRecordsLabel }}</p>
        </div>
      </div>
    </template>
  </SectionPanel>
...
defineProps({
  ...
  columnsLabel: { type: String, default: "" },
  columnsSummaryLabel: { type: String, default: "" },
  columnHint: { type: String, default: "" },
  showAllColumnsLabel: { type: String, default: "" },
  loadingLabel: { type: String, default: "" },
  loadErrorTitle: { type: String, default: "" },
  emptyTitle: { type: String, default: "" },
  emptyDescription: { type: String, default: "" },
  showingRecordsLabel: { type: String, default: "" },
  exportingLabel: { type: String, default: "" },
  ...
  showAllColumns: { type: Function, required: true },
  ...
  getSortIndicator: { type: Function, required: true },
  isRowClickable: { type: Function, required: true },
