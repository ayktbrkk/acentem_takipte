  ...
  <div v-if="modelValue" class="dialog-overlay" @click.self="close">
    <div class="dialog-shell dialog-md">
      ...
      <div class="dialog-footer">
        ...
        <button type="button" class="btn btn-outline" :disabled="loading || saveDisabledComputed" @click="submit(false)">{{ resolvedLabels.save }}</button>
        <button type="button" class="btn btn-primary" :disabled="loading || saveDisabledComputed" @click="submit(true)">{{ resolvedLabels.saveAndOpen }}</button>
      </div>
    </div>
  </div>
</template>
...
const resolvedLabels = computed(() => ({
  ...
  saveAndOpen: props.labels.saveAndOpen || (props.locale === "tr" ? "Olustur ve Ac" : "Create & Open"),
}));
...
async function submit(openAfter = false) {
  ...
  if (hasPolicyHolderField && !String(form.policy_holder || "").trim()) {
    errorText.value = props.locale === "tr" ? "Sigortali alani zorunludur." : "Policy holder is required.";
    fieldErrors.policy_holder = props.locale === "tr" ? "Sigortali" : "Policy Holder";
    return;
  }
