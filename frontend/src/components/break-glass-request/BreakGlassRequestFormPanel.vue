    ...
    <form class="space-y-4" @submit.prevent="$emit('submit')">
      <div class="grid gap-3 md:grid-cols-2">
        ...
        <label class="flex flex-col gap-1 md:col-span-2">
          <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">{{ t("referenceName") }}</span>
          <input v-model.trim="form.referenceName" class="input" type="text" :placeholder="t('referenceNamePlaceholder')" />
        </label>
      </div>

      <label class="flex flex-col gap-1">
        <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">{{ t("justification") }}</span>
        <textarea
