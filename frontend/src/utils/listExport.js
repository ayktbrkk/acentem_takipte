...
export function openListExport({ screen, query, format = "xlsx", limit = 1000, filename = "" }) {
  ...
  window.open(
    `/api/method/acentem_takipte.acentem_takipte.platform.api.list_exports.export_screen_list?${params.toString()}`,
    "_blank",
    "noopener",
  );
}
...
export function openTabularExport({
  ...
  submitExportForm("/api/method/acentem_takipte.acentem_takipte.platform.api.list_exports.export_tabular_payload", {
