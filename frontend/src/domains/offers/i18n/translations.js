export const OFFER_TRANSLATIONS = {
  tr: {
    title: "Teklif Yönetimi",
    subtitle: "Teklif listesini filtreleyin, sıralayın ve sayfalayın",
    breadcrumb: "Sigorta Operasyonları → Teklifler",
    recordCount: "kayıt",
    viewMode: "Görünüm modu",
    viewList: "Liste",
    viewBoard: "Pano",
    newOffer: "Yeni Teklif",
    summaryTotal: "Toplam Teklif",
    summaryDraft: "Taslak",
    summarySent: "Gönderildi",
    summaryAccepted: "Kabul Edildi",
    summaryConversion: "Dönüşüm Oranı",
    searchPlaceholder: "Teklif / Müşteri / Kayıt ara",
    filtersTitle: "Filtreler",
    customerName: "Müşteri",
    premiumAmount: "Prim",
    company: "Şirket",
    validUntil: "Geçerlilik",
    grossPremium: "Brüt Prim",
    netPremiumShort: "Net Prim",
    commissionShort: "Komisyon",
    recordId: "Kayıt No",
    offerDate: "Teklif Tarihi",
    advanced_filters_placeholder: "Gelişmiş filtreleme seçenekleri yakında eklenecek.",
    offerTableTitle: "Teklif Listesi",
    showingRecords: "Gösterilen kayıt",
    previousPage: "Önceki sayfa",
    nextPage: "Sonraki sayfa",
    pipelineTitle: "Teklif Panosu",
    draftLane: "Taslak",
    draftLaneHint: "Taslak teklifler",
    sentLane: "Gönderildi",
    sentLaneHint: "İletilen teklifler",
    acceptedLane: "Kabul Edildi",
    acceptedLaneHint: "Kabul edilen teklifler",
    convertedLane: "Poliçeleşti",
    convertedLaneHint: "Poliçeleşen teklifler",
    emptyTitle: "Teklif bulunamadı",
    empty: "Henüz teklif kaydı yok.",
    loading: "Teklifler yükleniyor...",
    loadErrorTitle: "Teklifler yüklenemedi",
    loadError: "Teklif listesi yüklenirken bir hata oluştu. Lütfen tekrar deneyin.",
    stage: "Aşama",
    emptyLane: "Bu aşamada teklif yok",
    convert: "Poliçeye Çevir",
    convert_to_policy: "Poliçeye Çevir",
    convert_confirm: "Bu teklif poliçeye dönüştürülsün mü?",
    convert_success: "Teklif poliçeye dönüştürüldü.",
    convert_failed: "Dönüştürme başarısız oldu.",
    openPolicy: "Poliçeyi Aç",
    quickOfferTitle: "Hızlı Teklif Oluştur",
    convertDialogTitle: "Teklifi Poliçeye Çevir",
    selectedOffer: "Seçilen Teklif",
    offers: "Teklifler",
    offer_detail: "Teklif Detayı",
    offers_breadcrumb: "Sigorta Operasyonları → Teklifler",
    total_offers: "Toplam Teklif",
    new_offer: "Yeni Teklif",
    refresh: "Yenile",
    filters: "Filtreler",
    offer_list: "Teklif Listesi",
    all: "Hepsi",
    overview: "Genel Bakış",
    customer_details: "Müşteri Detayları",
    policy_details: "Poliçe Detayları",
    activity: "Aktivite",
    timeline: "Zaman Çizelgesi",
    documents: "Dosyalar",
    no_activities: "Aktivite bulunamadı",
    no_documents: "Henüz doküman yüklenmedi.",
    records: "Kayıtlar",
    back_to_list: "Listeye Dön",
    status_draft: "Taslak",
    status_sent: "Gönderildi",
    status_accepted: "Kabul Edildi",
    status_rejected: "Reddedildi",
    status_converted: "Poliçeleşti",
    status_cancelled: "İptal",
    statusDraft: "Taslak",
    statusSent: "Gönderildi",
    statusAccepted: "Kabul Edildi",
    statusConverted: "Poliçeleşti",
    statusRejected: "Reddedildi",
    offer_no: "Teklif No",
    customer: "Müşteri",
    insurance_company: "Sigorta Şirketi",
    branch: "Branş",
    gross_premium: "Brüt Prim",
    net_premium: "Net Prim",
    status: "Durum",
    offer_date: "Teklif Tarihi",
    valid_until: "Geçerlilik Tarihi",
    assigned_agent: "Atanan Temsilci",
    converted_policy: "Poliçe No",
    related_offers: "İlgili Teklifler",
    related_policies: "İlgili Poliçeler",
    showing: "Gösterilen",
    total: "Toplam",
    active: "Aktif",
    open: "Açık",
    converted: "Poliçeleşen",
    cancelled: "İptal",
    offer_profile: "Teklif Profili",
    financial_details: "Finansal Detaylar",
    tax_amount: "Vergi Tutarı",
    commission_amount: "Komisyon Tutarı",
    validity: "Geçerlilik",
    unspecified: "Belirtilmedi",
    corporate: "Kurumsal",
    individual: "Bireysel",
    daysRemaining: "gün",
    offer_details: "Teklif Detayları",
    presetActionable: "Sadece aksiyon alınabilir teklifler",
    createPolicy: "Poliçe Oluştur",
    netPremium: "Net Prim",
    taxAmount: "Vergi Tutarı",
    commissionAmount: "Komisyon Tutarı",
    sortValidUntilAsc: "Geçerlilik (Artan)",
    sortValidUntilDesc: "Geçerlilik (Azalan)",
    sortGrossPrimaryDesc: "Prime Göre",
    presetConverted: "Dönüşenler",
    presetExpiring7: "7 Gün İçinde Bitecek",
    colOffer: "Teklif",
    colCustomer: "Müşteri",
    colValidity: "Geçerlilik",
    colPremium: "Prim",
    colStatus: "Durum",
    rangeMin: "En Az",
    rangeMax: "En Çok",
    customerSearchFailed: "Müşteri araması başarısız oldu",
    quickCreateValidationFailed: "Zorunlu alanları doldurun",
    quickOfferCreateFailed: "Teklif oluşturulamadı",
    conversionFailed: "Dönüşüm başarısız oldu",
    statusUpdateFailed: "Durum güncellenemedi",
    exportXlsx: "Excel'e Aktar",
    clearFilters: "Filtreleri Temizle",
    applyFilters: "Filtreleri Uygula",
    cancel: "İptal",
    policyNo: "Poliçe No",
    start_date: "Başlangıç Tarihi",
    end_date: "Bitiş Tarihi",
    phone: "Telefon",
    email: "E-posta",
    tax_id: "TCKN/VKN",
    activity_timeline: "İşlem Geçmişi",
    no_recent_activity: "Henüz bir işlem kaydı bulunmuyor.",
    upload: "Yükle",
    view_all_documents: "Tüm Dokümanları Görüntüle",
    openDocument: "Dokümanı Aç",
    save_success: "Değişiklikler başarıyla kaydedildi.",
    save_failed: "Değişiklikler kaydedilemedi.",
    createQuickOffer: "Kaydet",
    createQuickOfferAndOpen: "Kaydet ve Aç",
  },
  en: {
    title: "Offer Management",
    subtitle: "Filter, sort, and paginate offers",
    breadcrumb: "Insurance Operations → Offers",
    recordCount: "records",
    viewMode: "View mode",
    viewList: "List",
    viewBoard: "Board",
    newOffer: "New Offer",
    summaryTotal: "Total Offers",
    summaryDraft: "Draft",
    summarySent: "Sent",
    summaryAccepted: "Accepted",
    summaryConversion: "Conversion Rate",
    searchPlaceholder: "Search offers / customers / records",
    filtersTitle: "Filters",
    customerName: "Customer",
    premiumAmount: "Premium",
    company: "Company",
    validUntil: "Validity",
    grossPremium: "Gross Premium",
    netPremiumShort: "Net Premium",
    commissionShort: "Commission",
    recordId: "Record ID",
    offerDate: "Offer Date",
    advanced_filters_placeholder: "Advanced filtering options will be added soon.",
    offerTableTitle: "Offer List",
    showingRecords: "Showing records",
    previousPage: "Previous page",
    nextPage: "Next page",
    pipelineTitle: "Offer Board",
    draftLane: "Draft",
    draftLaneHint: "Draft offers",
    sentLane: "Sent",
    sentLaneHint: "Submitted offers",
    acceptedLane: "Accepted",
    acceptedLaneHint: "Accepted offers",
    convertedLane: "Converted",
    convertedLaneHint: "Converted offers",
    emptyTitle: "No offers found",
    empty: "There are no offers yet.",
    loading: "Loading offers...",
    loadErrorTitle: "Failed to load offers",
    loadError: "An error occurred while loading the offer list. Please try again.",
    stage: "Stage",
    emptyLane: "No offers in this stage",
    convert: "Convert to Policy",
    convert_to_policy: "Convert to Policy",
    convert_confirm: "Convert this offer to a policy?",
    convert_success: "Offer converted to policy.",
    convert_failed: "Conversion failed.",
    openPolicy: "Open Policy",
    quickOfferTitle: "Quick Create Offer",
    convertDialogTitle: "Convert Offer to Policy",
    selectedOffer: "Selected Offer",
    offers: "Offers",
    offer_detail: "Offer Detail",
    offers_breadcrumb: "Insurance Operations → Offers",
    total_offers: "Total Offers",
    new_offer: "New Offer",
    refresh: "Refresh",
    filters: "Filters",
    offer_list: "Offer List",
    all: "All",
    overview: "Overview",
    customer_details: "Customer Details",
    policy_details: "Policy Details",
    activity: "Activity",
    timeline: "Timeline",
    documents: "Documents",
    no_activities: "No activities found",
    no_documents: "No documents uploaded yet.",
    records: "Records",
    back_to_list: "Back to List",
    status_draft: "Draft",
    status_sent: "Sent",
    status_accepted: "Accepted",
    status_rejected: "Rejected",
    status_converted: "Converted",
    status_cancelled: "Cancelled",
    statusDraft: "Draft",
    statusSent: "Sent",
    statusAccepted: "Accepted",
    statusConverted: "Converted",
    statusRejected: "Rejected",
    offer_no: "Offer No",
    customer: "Customer",
    insurance_company: "Insurance Company",
    branch: "Branch",
    gross_premium: "Gross Premium",
    net_premium: "Net Premium",
    status: "Status",
    offer_date: "Offer Date",
    valid_until: "Valid Until",
    assigned_agent: "Assigned Agent",
    converted_policy: "Policy No",
    related_offers: "Related Offers",
    related_policies: "Related Policies",
    showing: "Showing",
    total: "Total",
    active: "Active",
    open: "Open",
    converted: "Converted",
    cancelled: "Cancelled",
    offer_profile: "Offer Profile",
    financial_details: "Financial Details",
    tax_amount: "Tax Amount",
    commission_amount: "Commission Amount",
    validity: "Validity",
    unspecified: "Not provided",
    corporate: "Corporate",
    individual: "Individual",
    daysRemaining: "days",
    offer_details: "Offer Details",
    presetActionable: "Actionable offers only",
    createPolicy: "Create Policy",
    netPremium: "Net Premium",
    taxAmount: "Tax Amount",
    commissionAmount: "Commission Amount",
    sortValidUntilAsc: "Validity (Ascending)",
    sortValidUntilDesc: "Validity (Descending)",
    sortGrossPrimaryDesc: "By Premium",
    presetConverted: "Converted",
    presetExpiring7: "Expiring in 7 Days",
    colOffer: "Offer",
    colCustomer: "Customer",
    colValidity: "Validity",
    colPremium: "Premium",
    colStatus: "Status",
    rangeMin: "Min",
    rangeMax: "Max",
    customerSearchFailed: "Customer search failed",
    quickCreateValidationFailed: "Please fill in required fields",
    quickOfferCreateFailed: "Failed to create offer",
    conversionFailed: "Conversion failed",
    statusUpdateFailed: "Status update failed",
    exportXlsx: "Export Excel",
    clearFilters: "Clear Filters",
    applyFilters: "Apply Filters",
    cancel: "Cancel",
    policyNo: "Policy No",
    start_date: "Start Date",
    end_date: "End Date",
    phone: "Phone",
    email: "Email",
    tax_id: "Tax ID",
    activity_timeline: "Activity Timeline",
    no_recent_activity: "No recent activity recorded yet.",
    upload: "Upload",
    view_all_documents: "View All Documents",
    openDocument: "Open Document",
    save_success: "Changes saved successfully.",
    save_failed: "Failed to save changes.",
    createQuickOffer: "Save",
    createQuickOfferAndOpen: "Save & Open",
  }
};

