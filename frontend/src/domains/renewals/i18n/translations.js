export const RENEWAL_TRANSLATIONS = {
  tr: {
    title: "Yenileme Yönetimi",
    renewals: "Yenilemeler",
    renewal_detail: "Yenileme Detayı",
    renewals_breadcrumb: "Sigorta Operasyonları → Yenilemeler",
    total_renewals: "Toplam Yenileme",
    new_renewal: "Yeni Yenileme Görevi",
    refresh: "Yenile",
    filters: "Filtreler",
    renewal_list: "Yenileme Listesi",
    all: "Tümü",
    overview: "Genel Bakış",
    customer_details: "Müşteri Detayları",
    activity: "Aktivite",
    timeline: "Zaman Çizelgesi",
    documents: "Dosyalar",
    no_activities: "Aktivite bulunamadı",
    back_to_list: "Listeye Dön",
    status_open: "Açık",
    status_active: "Aktif",
    status_in_progress: "Takipte",
    status_done: "Tamamlandı",
    status_cancelled: "İptal",
    renewal_date: "Yenileme Tarihi",
    due_date: "Termin Tarihi",
    policy_no: "Poliçe No",
    insurance_company: "Sigorta Şirketi",
    branch: "Branş",
    showing: "Gösterilen",
    status: "Durum",
    total: "Toplam",
    active: "Aktif",
    completed: "Tamamlanan",
    cancelled: "İptal",
    priority: "Öncelik",
    priority_high: "Yüksek",
    priority_medium: "Orta",
    priority_low: "Düşük",
    viewMode: "Görünüm",
    viewList: "Liste",
    viewBoard: "Pano",
    colPolicy: "Poliçe",
    colCustomer: "Müşteri",
    colBranch: "Branş",
    colDueDate: "Vade",
    colStatus: "Durum",
    colPriority: "Öncelik",
    open_policy: "Poliçeyi Aç",
    mark_in_progress: "Takibe Al",
    mark_done: "Tamamla",
    task: "Görev",
    due: "Vade",
    policy: "Poliçe",
    new_offers: "Yeni Teklifler",
    communication_history: "İletişim Geçmişi",
    // Kanban & Details
    breadcrumb: "Yenilemeler",
    backList: "Listeye Dön",
    renewalProcess: "Yenileme Süreci",
    previousPolicy: "Eski Poliçe Bilgileri",
    reminders: "Hatırlatıcılar",
    people: "İlgili Kişiler",
    statusInfo: "Durum Bilgisi",
    recordMeta: "Kayıt Meta",
    noPolicy: "Poliçe bilgisi bulunamadı.",
    noOffers: "Bu poliçe için teklif bulunamadı.",
    noCommunication: "İletişim kaydı bulunamadı.",
    noReminders: "Hatırlatıcı kaydı bulunamadı.",
    openOfferDetail: "Teklif Detayını Aç",
    stepNotified: "Bildirim",
    stepOfferSent: "Teklif",
    stepDecision: "Karar",
    stepRenewed: "Poliçe",
    lblPolicy: "Poliçe",
    lblDue: "Vade",
    lblRenewalDate: "Yenileme Tarihi",
    lblStatus: "Durum",
    fldPolicyNo: "Poliçe No",
    fldCustomer: "Müşteri",
    fldBranch: "Branş",
    fldEndDate: "Bitiş",
    fldAgent: "Temsilci",
    fldPriority: "Öncelik",
    fldLostReason: "Kayıp Nedeni",
    fldCreatedBy: "Oluşturan",
    fldCreated: "Oluşturma",
    fldModifiedBy: "Güncelleyen",
    fldModified: "Güncelleme",
    fldDueDate: "Son Tarih",
    fldRenewalDateLbl: "Yenileme Tarihi",
    fldNextContact: "Sonraki İletişim",
    subtitle: "Bitişe yakın poliçeler",
    recordCount: "kayıt",
    newTask: "Yeni Görev",
    newRenewalTask: "Yeni Yenileme Görevi",
    quickRenewalEyebrow: "Hızlı Yenileme",
    exportExcel: "Excel",
    exportPdf: "PDF",
    searchPlaceholder: "Görev / poliçe ara",
    filtersTitle: "Filtreler",
    showingRecords: "Gösterilen kayıt",
    allStatuses: "Tüm Durumlar",
    allDueScopes: "Tüm Vadeler",
    dueScopeOverdue: "Gecikmiş",
    dueScope7: "7 Gün",
    dueScope30: "30 Gün",
    policySearchLabel: "Poliçe No",
    clearFilters: "Filtreleri Temizle",
    applyFilters: "Uygula",
    emptyList: "Henüz yenileme kaydı yok.",
    emptyBoardColumn: "Kart yok",
    columnOpen: "Açık",
    columnOpenHint: "Yeni yenilemeler",
    columnInProgress: "Takipte",
    columnInProgressHint: "İşlemdeki kayıtlar",
    columnDone: "Tamamlandı",
    columnDoneHint: "Tamamlanan kayıtlar",
    columnCancelled: "İptal",
    columnCancelledHint: "İptal edilenler",
    metricTotal: "Toplam",
    metricOpen: "Açık",
    metricInProgress: "Takipte",
    metricDone: "Tamamlandı",
    metricCancelled: "İptal",
    metricDue: "Vadesi Gelen",
    loadError: "Yenileme kayıtları yüklenemedi.",
    retry: "Tekrar Dene",
    fetchLimitWarning: "İlk 500 yenileme kaydı gösteriliyor. Daha fazlası için filtreleri daraltın veya dışa aktarın.",
    presetLabel: "Filtre Şablonu",
    savePreset: "Şablonu Kaydet",
    deletePreset: "Şablonu Sil",
    unspecified: "Belirtilmedi",
    competitor: "Kayıp Nedeni",
    renewal: "Yenileme",
    detailSubtitle: "Yenileme görevini takip edin, ilgili poliçeyi kontrol edin ve süreci netleştirin",
    priorityCritical: "Kritik",
    prioritySoon: "Yaklaşan",
    priorityNormal: "Planlandı",
    priorityDone: "Tamamlandı",
    priorityCancelled: "İptal",
    priorityUnknown: "Belirsiz",
  },
  en: {
    title: "Renewal Management",
    renewals: "Renewals",
    renewal_detail: "Renewal Detail",
    renewals_breadcrumb: "Insurance Operations → Renewals",
    total_renewals: "Total Renewals",
    new_renewal: "New Renewal Task",
    refresh: "Refresh",
    filters: "Filters",
    renewal_list: "Renewal List",
    all: "All",
    overview: "Overview",
    customer_details: "Customer Details",
    activity: "Activity",
    timeline: "Timeline",
    documents: "Documents",
    no_activities: "No activities found",
    back_to_list: "Back to List",
    status_open: "Open",
    status_active: "Active",
    status_in_progress: "In Progress",
    status_done: "Done",
    status_cancelled: "Cancelled",
    renewal_date: "Renewal Date",
    due_date: "Due Date",
    policy_no: "Policy No",
    insurance_company: "Insurance Company",
    branch: "Branch",
    showing: "Showing",
    status: "Status",
    total: "Total",
    active: "Active",
    completed: "Completed",
    cancelled: "Cancelled",
    priority: "Priority",
    priority_high: "High",
    priority_medium: "Medium",
    priority_low: "Low",
    viewMode: "View",
    viewList: "List",
    viewBoard: "Board",
    colPolicy: "Policy",
    colCustomer: "Customer",
    colBranch: "Branch",
    colDueDate: "Due Date",
    colStatus: "Status",
    colPriority: "Priority",
    open_policy: "Open Policy",
    mark_in_progress: "Start Follow-up",
    mark_done: "Mark Done",
    task: "Task",
    due: "Due",
    policy: "Policy",
    new_offers: "New Offers",
    communication_history: "Communication History",
    // Kanban & Details
    breadcrumb: "Renewals",
    backList: "Back to List",
    renewalProcess: "Renewal Process",
    previousPolicy: "Previous Policy",
    reminders: "Reminders",
    people: "People",
    statusInfo: "Status",
    recordMeta: "Record Info",
    noPolicy: "No policy information found.",
    noOffers: "No offers found for this policy.",
    noCommunication: "No communication records found.",
    noReminders: "No reminder records found.",
    openOfferDetail: "Open Offer Detail",
    stepNotified: "Notified",
    stepOfferSent: "Offer",
    stepDecision: "Decision",
    stepRenewed: "Policy",
    lblPolicy: "Policy",
    lblDue: "Due",
    lblRenewalDate: "Renewal Date",
    lblStatus: "Status",
    fldPolicyNo: "Policy No",
    fldCustomer: "Customer",
    fldBranch: "Branch",
    fldEndDate: "End Date",
    fldAgent: "Agent",
    fldPriority: "Priority",
    fldLostReason: "Lost Reason",
    fldCreatedBy: "Created By",
    fldCreated: "Created",
    fldModifiedBy: "Modified By",
    fldModified: "Modified",
    fldDueDate: "Due Date",
    fldRenewalDateLbl: "Renewal Date",
    fldNextContact: "Next Contact",
    subtitle: "Near-expiry policies",
    recordCount: "records",
    newTask: "New Task",
    newRenewalTask: "New Renewal Task",
    quickRenewalEyebrow: "Quick Renewal",
    exportExcel: "Excel",
    exportPdf: "PDF",
    searchPlaceholder: "Search task / policy",
    filtersTitle: "Filters",
    showingRecords: "Showing records",
    allStatuses: "All Statuses",
    allDueScopes: "All Due Ranges",
    dueScopeOverdue: "Overdue",
    dueScope7: "7 Days",
    dueScope30: "30 Days",
    policySearchLabel: "Policy No",
    clearFilters: "Clear filters",
    applyFilters: "Apply filters",
    emptyList: "There are no renewal records yet.",
    emptyBoardColumn: "No cards",
    columnOpen: "Open",
    columnOpenHint: "New renewals",
    columnInProgress: "In Progress",
    columnInProgressHint: "Ongoing tasks",
    columnDone: "Done",
    columnDoneHint: "Completed tasks",
    columnCancelled: "Cancelled",
    columnCancelledHint: "Cancelled tasks",
    metricTotal: "Total",
    metricOpen: "Open",
    metricInProgress: "In Progress",
    metricDone: "Done",
    metricCancelled: "Cancelled",
    metricDue: "Due Soon",
    loadError: "Renewal records could not be loaded.",
    retry: "Retry",
    fetchLimitWarning: "Showing the first 500 renewal records. Narrow filters or export for more.",
    presetLabel: "Filter Preset",
    savePreset: "Save Preset",
    deletePreset: "Delete Preset",
    unspecified: "Not provided",
    competitor: "Lost Reason",
    renewal: "Renewal",
    detailSubtitle: "Track the renewal task, inspect the linked policy, and keep the process clear",
    priorityCritical: "Critical",
    prioritySoon: "Upcoming",
    priorityNormal: "Scheduled",
    priorityDone: "Completed",
    priorityCancelled: "Cancelled",
    priorityUnknown: "Unknown",
  }
};
