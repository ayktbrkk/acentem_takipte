...
export function useSidebarNavigation() {
  ...
  const menuLabel = computed(() => t("menu"));
  const brandLabel = computed(() => t("brand"));
  const subtitleLabel = computed(() => t("subtitle"));
  const miniTitleLabel = computed(() => t("miniTitle"));
  const collapseSidebarLabel = computed(() => t("collapseSidebar"));
  const expandSidebarLabel = computed(() => t("expandSidebar"));
  const collapseShortLabel = computed(() => t("collapseShort"));
  const expandShortLabel = computed(() => t("expandShort"));
  const sidebarToggleTitle = computed(() =>
    isCollapsed.value ? expandSidebarLabel.value : collapseSidebarLabel.value
  );
  const sidebarToggleShortLabel = computed(() =>
    isCollapsed.value ? expandShortLabel.value : collapseShortLabel.value
  );
  ...
  const branchLabel = computed(
    () => String(branchStore.requestBranch || authStore.defaultOfficeBranch || "-").trim() || "-"
  );
  ...
  const navSections = computed(() => [
    { title: t("sectionOverview"), items: [{ key: "dashboard", label: t("dashboard"), to: "/", short: "DB", badgeClass: "text-emerald-700" }] },
    {
