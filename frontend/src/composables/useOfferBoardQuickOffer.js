...
export function useOfferBoardQuickOffer({
  ...
  buildQuickCreateIntentQuery: buildIntentQuery = buildQuickCreateIntentQuery,
  buildRelatedQuickCreateNavigation: buildRelatedNavigation = buildRelatedQuickCreateNavigation,
  branches,
  createQuickCreateDraftConfig,
  getLocalizedText,
  isValidTckn,
  ...
  const offerQuickOptionsMap = computed(() => ({
    ...
    insuranceCompanies: insuranceCompanies.value,
    salesEntities: salesEntities.value,
  }));
  ...
  function buildQuickOfferReturnTo() {
    ...
    return router.resolve({
      name: "offer-board",
      query: buildIntentQuery({ prefills }),
    }).fullPath;
  }

  function onOfferRelatedCreateRequested(request = {}) {
    const navigation = buildRelatedNavigation({
