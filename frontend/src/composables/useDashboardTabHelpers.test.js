...
describe("useDashboardTabHelpers", () => {
  it("derives sorted renewal, follow-up, and activity lists", () => {
    const helpers = useDashboardTabHelpers({
      ...
      followUpResource: { data: ref({ summary: { total: 2, overdue: 1, due_today: 1, due_soon: 0 }, meta: { settings: { follow_up_due_soon_days: 7, follow_up_preview_limit: 8 } }, items: [{ name: "F2", days_delta: 4 }, { name: "F1", days_delta: -1 }] }) },
    ...
    expect(helpers.followUpMeta.value.settings).toEqual({ follow_up_due_soon_days: 7, follow_up_preview_limit: 8 });
