<template>
  <WorkbenchPageLayout
    ...
    :title="t('title')"
    :subtitle="t('subtitle')"
    :record-count="activeTab === 'settings' ? editableSettingCount : configuredChannelCount"
    :record-count-label="activeTab === 'settings' ? t('recordCountLabel') : t('alertRecordCountLabel')"
  >
    <template #metrics>
      <div v-if="activeTab === 'settings'" class="grid grid-cols-1 gap-4 md:grid-cols-4">
    ...
    <template #actions>
      <ActionButton v-if="activeTab === 'settings'" variant="secondary" size="sm" :disabled="!hasUnsavedChanges" @click="resetToDefaults">
        {{ t('resetToDefaults') }}
      </ActionButton>
      <ActionButton v-if="activeTab === 'alerts'" variant="secondary" size="sm" :disabled="alertChannelsLoading" @click="loadAlertChannels">
        {{ t('refresh') }}
      </ActionButton>
    </template>
        ...
        <div class="space-y-4">
          ...
          <SectionPanel :title="t('actionsTitle')" :meta="t('actionsSubtitle')" :show-count="false">
            <div class="flex flex-wrap gap-3">
              <ActionButton variant="primary" size="sm" :disabled="loading || saving || !hasUnsavedChanges" @click="saveSettings"><FeatherIcon v-if="saving" name="loader" class="h-4 w-4 animate-spin mr-1" />{{ saving ? t('savingButton') : t('saveButton') }}</ActionButton>
            </div>
          </SectionPanel>
        </div>
      </div>
...
const alertChannelConfig = ref({ slack_webhook_url: "", telegram_bot_token: "", telegram_chat_id: "", slack_configured: false, telegram_configured: false });
...
async function loadSettings() { loading.value = true; error.value = ""; try { const payload = await frappeRequest({ url: "/api/method/acentem_takipte.acentem_takipte.api.admin_settings.get_admin_general_settings", method: "GET" }); applyLoadedSettings(payload?.message || payload || {}); } catch (err) { error.value = String(err?.message || err || t("loadingError")); } finally { loading.value = false; } }

async function saveSettings() { saving.value = true; error.value = ""; try { const payload = await frappeRequest({ url: "/api/method/acentem_takipte.acentem_takipte.api.admin_settings.save_admin_general_settings_api", method: "POST", params: { config: buildSavePayload() } }); applyLoadedSettings(payload?.message || payload || settings.value); showToast(t('saveSuccess'), "success"); } catch (err) { error.value = String(err?.message || err || t("savingError")); showToast(t('savingError'), "error"); } finally { saving.value = false; } }

async function loadAlertChannels() { if (!canManageAlertChannels.value) return; alertChannelsLoading.value = true; alertError.value = ""; try { const payload = await frappeRequest({ url: "/api/method/acentem_takipte.acentem_takipte.api.reports.get_ops_alert_channel_settings", method: "GET" }); const m = payload?.message || payload || {}; alertChannelConfig.value = { slack_webhook_url: String(m.slack_webhook_url || ""), telegram_bot_token: String(m.telegram_bot_token || ""), telegram_chat_id: String(m.telegram_chat_id || ""), slack_configured: Boolean(m.slack_configured), telegram_configured: Boolean(m.telegram_configured) }; } catch (err) { alertError.value = String(err?.message || err || alertT("loadError")); } finally { alertChannelsLoading.value = false; } }

async function saveAlertChannels(config) { alertChannelsSaving.value = true; alertError.value = ""; try { const payload = await frappeRequest({ url: "/api/method/acentem_takipte.acentem_takipte.api.reports.save_ops_alert_channel_settings_api", method: "POST", params: { config } }); const m = payload?.message || payload || {}; alertChannelConfig.value = { slack_webhook_url: String(m.slack_webhook_url || ""), telegram_bot_token: String(m.telegram_bot_token || ""), telegram_chat_id: String(m.telegram_chat_id || ""), slack_configured: Boolean(m.slack_configured), telegram_configured: Boolean(m.telegram_configured) }; showToast(alertT("saveSuccess") || "Saved", "success"); } catch (err) { alertError.value = String(err?.message || err || alertT("saveError")); } finally { alertChannelsSaving.value = false; } }
