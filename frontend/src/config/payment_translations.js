export const PAYMENT_TRANSLATIONS = {
  tr: {
    payments: "Ödemeler",
    title: "Ödemeler",
    subtitle: "Ödeme akışını izleyin, tahsilatları yönetin ve operasyonu dengede tutun",
    recordCount: "kayıt",
    payment_detail: "Ödeme Detayı",
    payments_breadcrumb: "Sigorta Operasyonları → Ödemeler",
    total_payments: "Toplam Ödeme",
    summaryTotal: "Toplam Ödeme",
    summaryPending: "Bekleyen",
    summaryCollected: "Tahsil Edildi",
    summaryOverdue: "Gecikmiş",
    summaryTotalAmount: "Toplam Tutar",
    new_payment: "Yeni Ödeme",
    newPayment: "Tahsilat Kaydet",
    refresh: "Yenile",
    exportXlsx: "Excel",
    exportPdf: "PDF",
    filters: "Filtreler",
    payment_list: "Ödeme Listesi",
    all: "Tümü",
    allDirections: "Tüm Yönler",
    inbound: "Gelen",
    outbound: "Giden",
    advancedFilters: "Gelişmiş Filtreler",
    hideAdvancedFilters: "Gelişmiş Filtreleri Gizle",
    activeFilters: "aktif filtre",
    presetLabel: "Filtre Şablonu",
    savePreset: "Kaydet",
    deletePreset: "Sil",
    applyFilters: "Uygula",
    clearFilters: "Filtreleri Temizle",
    searchPlaceholder: "Ödeme / Poliçe / Müşteri ara",
    customerFilter: "Müşteri",
    policyFilter: "Poliçe",
    purposeFilter: "Amaç",
    sortModifiedDesc: "Son Güncellenen",
    sortPaymentDateDesc: "Ödeme Tarihi (Yeni)",
    sortPaymentDateAsc: "Ödeme Tarihi (Eski)",
    sortAmountDesc: "Tutar (Yüksek)",
    overview: "Genel Bakış",
    customer_details: "Müşteri Detayları",
    installments: "Taksitler",
    documents: "Dosyalar",
    no_activities: "Henüz bir işlem kaydı bulunmuyor.",
    no_documents: "Henüz doküman yüklenmedi.",
    back_to_list: "Listeye Dön",
    payment_date: "Ödeme Tarihi",
    payment_no: "Ödeme No",
    policy: "Poliçe",
    carrier_policy_no: "Şirket Poliçe No",
    branch: "Şube",
    due_date: "Termin Tarihi",
    collected: "Tahsil Edildi",
    remaining: "Kalan",
    actions: "İşlemler",
    collectPayment: "Tahsilat Kaydet",
    addReceipt: "Dekont Ekle",
    sendReminder: "Hatırlatma Gönder",
    customer_type: "Müşteri Tipi",
    tax_id_label: "TC/VKN",
    birth_date: "Doğum Tarihi",
    insurance_company: "Sigorta Şirketi",
    amount: "Tutar",
    currency: "Para Birimi",
    loadError: "Ödeme listesi yüklenemedi.",
    loadErrorTitle: "Ödemeler Yüklenemedi",
    emptyTitle: "Ödeme Bulunamadı",
    empty: "Ödeme kaydı bulunamadı.",
    loading: "Yükleniyor...",
    showing: "Gösterilen",
    showingRecords: "ödeme gösteriliyor",
    total: "Toplam",
    paid: "Ödendi",
    unpaid: "Ödenmedi",
    overdue: "Gecikmiş",
    payment_direction: "Yön",
    payment_purpose: "Amaç",
    quickPayment: "Hızlı Tahsilat",
    customer: "Müşteri",
    status: "Durum",
    activity_timeline: "İşlem Geçmişi",
    save_success: "Kayıt başarıyla güncellendi.",
    save_failed: "Güncelleme başarısız oldu. Lütfen tekrar deneyin.",
    unspecified: "Belirtilmedi",
    file_link_not_found: "Doküman bağlantısı bulunamadı.",
    status_overdue: "Gecikmiş",
    status_outstanding: "Bekleyen",
    status_partially_paid: "Kısmi Ödeme",
    status_cancelled: "İptal",
    status_unpaid: "Ödenmedi",
    status_paid: "Ödendi",
    status_partial: "Kısmi Ödeme",
    recordId: "Kayıt No",
    paidInstallments: "Ödenen Taksit",
    overdueInstallments: "Geciken Taksit",
    nextInstallmentDue: "Sıradaki Vade",
    view_full_profile: "Profilin Tamamını Gör",
    receipt_title: "Dekont / Fatura",
    openDocumentCenter: "Doküman Merkezine Git",
    openDocument: "Dokümanı Aç",
    payment_info: "Ödeme Bilgileri",
    financial_summary: "Finansal Özet",
    payment_plan: "Ödeme Planı",
    installment_no: "Taksit No",
    claim_detail: "Hasar Detayı",
    colPayment: "Ödeme / Poliçe",
    colCustomer: "Müşteri Bilgileri",
    colDueDate: "Vade / Durum",
    colFinance: "Finansal Durum",
    colInstallmentNo: "Taksit No",
    colTerminDate: "Termin Tarihi",
    colAmount: "Tutar",
    colStatus: "Durum",
  },
  en: {
    payments: "Payments",
    title: "Payments",
    subtitle: "Track payment flow, manage collections, and keep operations balanced",
    recordCount: "records",
    payment_detail: "Payment Detail",
    payments_breadcrumb: "Insurance Operations → Payments",
    total_payments: "Total Payments",
    summaryTotal: "Total Payments",
    summaryPending: "Pending",
    summaryCollected: "Collected",
    summaryOverdue: "Overdue",
    summaryTotalAmount: "Total Amount",
    new_payment: "New Payment",
    newPayment: "Collect Payment",
    refresh: "Refresh",
    exportXlsx: "Excel",
    exportPdf: "PDF",
    filters: "Filters",
    payment_list: "Payment List",
    all: "All",
    allDirections: "All Directions",
    inbound: "Inbound",
    outbound: "Outbound",
    advancedFilters: "Advanced Filters",
    hideAdvancedFilters: "Hide Advanced Filters",
    activeFilters: "active filters",
    presetLabel: "Filter Preset",
    savePreset: "Save",
    deletePreset: "Delete",
    applyFilters: "Apply",
    clearFilters: "Clear Filters",
    searchPlaceholder: "Search payment / policy / customer",
    customerFilter: "Customer",
    policyFilter: "Policy",
    purposeFilter: "Purpose",
    sortModifiedDesc: "Recently Updated",
    sortPaymentDateDesc: "Payment Date (Newest)",
    sortPaymentDateAsc: "Payment Date (Oldest)",
    sortAmountDesc: "Amount (High)",
    overview: "Overview",
    customer_details: "Customer Details",
    installments: "Installments",
    documents: "Documents",
    no_activities: "No recent activity yet.",
    no_documents: "No documents uploaded yet.",
    back_to_list: "Back to List",
    payment_date: "Payment Date",
    payment_no: "Payment No",
    policy: "Policy",
    carrier_policy_no: "Carrier Policy No",
    branch: "Branch",
    due_date: "Due Date",
    collected: "Collected",
    remaining: "Remaining",
    actions: "Actions",
    collectPayment: "Collect Payment",
    addReceipt: "Add Receipt",
    sendReminder: "Send Reminder",
    customer_type: "Customer Type",
    tax_id_label: "Tax ID",
    birth_date: "Birth Date",
    insurance_company: "Insurance Company",
    amount: "Amount",
    currency: "Currency",
    loadError: "Payment list could not be loaded.",
    loadErrorTitle: "Payments Could Not Load",
    emptyTitle: "No Payments Found",
    empty: "No payment records found.",
    loading: "Loading...",
    showing: "Showing",
    showingRecords: "payments shown",
    total: "Total",
    paid: "Paid",
    unpaid: "Unpaid",
    overdue: "Overdue",
    payment_direction: "Direction",
    payment_purpose: "Purpose",
    quickPayment: "Quick Payment",
    customer: "Customer",
    status: "Status",
    activity_timeline: "Activity History",
    save_success: "Record updated successfully.",
    save_failed: "Update failed. Please try again.",
    unspecified: "Not provided",
    file_link_not_found: "Document link not found.",
    status_overdue: "Overdue",
    status_outstanding: "Outstanding",
    status_partially_paid: "Partially Paid",
    status_cancelled: "Cancelled",
    status_unpaid: "Unpaid",
    status_paid: "Paid",
    status_partial: "Partial",
    recordId: "Record ID",
    paidInstallments: "Paid Installments",
    overdueInstallments: "Overdue Installments",
    nextInstallmentDue: "Next Due Date",
    view_full_profile: "View Full Profile",
    receipt_title: "Receipt / Invoice",
    openDocumentCenter: "Go to Document Center",
    openDocument: "Open Document",
    payment_info: "Payment Info",
    financial_summary: "Financial Summary",
    payment_plan: "Payment Plan",
    installment_no: "Installment No",
    claim_detail: "Claim Detail",
    colPayment: "Payment / Policy",
    colCustomer: "Customer Info",
    colDueDate: "Due Date / Status",
    colFinance: "Financial Status",
    colInstallmentNo: "Installment No",
    colTerminDate: "Due Date",
    colAmount: "Amount",
    colStatus: "Status",
  }
};


