...
from acentem_takipte.acentem_takipte.domains.customers.services.customer_segments import (
    build_customer_segment_snapshot_payload,
    upsert_customer_segment_snapshot,
)
from acentem_takipte.acentem_takipte.platform.services.document_center import (
...
def _get_customer_files(customer_name: str) -> list[dict[str, Any]]:
    ...
    # Batch File lookup
    file_names = [d["file"] for d in merged if d.get("file")]
    if file_names:
        ...
        file_map: dict[str, Any] = {r["name"]: r for r in file_rows}
    else:
        file_map = {}

    for doc in merged:
        file_rec = file_map.get(doc.get("file") or "") or {}
        doc["file_name"] = file_rec.get("file_name") or doc.get("file") or ""
        doc["file_url"] = file_rec.get("file_url") or ""
        doc["file_size"] = file_rec.get("file_size")
        doc["is_private"] = file_rec.get("is_private")
