...
def can_view_break_glass_audit(user: str | None = None) -> bool:
    ...
    if isinstance(configured_roles, str) and configured_roles.strip():
        allowed_roles.update(role.strip() for role in configured_roles.split(",") if role.strip())
    elif isinstance(configured_roles, (list, tuple, set)):
        allowed_roles.update(str(role).strip() for role in configured_roles if str(role).strip())

    return bool(allowed_roles.intersection(set(frappe.get_roles(user_id))))


def is_break_glass_active(user: str | None = None, reference_doctype: str = "AT Claim") -> bool:
...
def create_break_glass_request(
    ...
    if existing > 0:
        frappe.throw(
            frappe._("You already have a pending request for {0} access").format(access_type),
    ...
    return {
        ...
        "request_id": doc.name,
        "status": "Pending",
        "message": frappe._("Break-glass request submitted for approval. You will be notified when reviewed.")
    }
...
def approve_break_glass_request(
    ...
    return {
        ...
        "request_id": request_id,
        "expires_at": str(expires_at),
        "message": frappe._("Break-glass request approved. Access valid until {0}").format(expires_at)
    }
...
def validate_break_glass_access(
    ...
    try:
        ...
        if now_dt > expires_at:
            # Mark as expired
            frappe.db.set_value("AT Break Glass Request", request_doc.name, "status", "Expired")
            return False, None
...
def expire_break_glass_grants():
    ...
    try:
        ...
        for doc in expired_requests:
            try:
                frappe.db.set_value("AT Break Glass Request", doc.name, "status", "Expired")
            except Exception as e:
    ...
    anomalies = [
        {
            ...
            "first_request_at": str(row.first_request_at) if row.first_request_at else None,
...
def run_break_glass_audit_monitor() -> dict:
    ...
    try:
        ...
        window_hours = int(site_config.get("at_break_glass_anomaly_window_hours", 48) or 48)
        threshold = int(site_config.get("at_break_glass_anomaly_threshold", 3) or 3)
        summary = detect_break_glass_anomalies(window_hours=window_hours, threshold=threshold)
