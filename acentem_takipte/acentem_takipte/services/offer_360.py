...
LOGGER = get_logger("acentem_takipte.offer_360")


def build_offer_360_payload(name: str) -> dict:
    offer_name = str(name or "").strip()
    if not offer_name:
        return {}

    # Try to get from cache first (5 min TTL)
    cache_key = f"at_offer_360:{offer_name}"
    cached_payload = frappe.cache().get_value(cache_key)
    if cached_payload:
        return cached_payload

    offer_doc = frappe.get_doc("AT Offer", offer_name)
    offer = offer_doc.as_dict(no_default_fields=False)
    
    # Enrich payload with related data
    payload = {
        "offer": offer,
        "customer": _get_customer(offer.get("customer")),
        "related_offers": _get_related_offers(offer),
        "related_policies": _get_related_policies(offer),
        "activity": _get_activity(offer),
        "notification_drafts": _get_rows(
            "AT Notification Draft",
            fields=["name", "creation", "channel", "language", "status", "subject", "body"],
            filters={"reference_doctype": "AT Offer", "reference_name": offer_name},
        ...
        "payments": _get_rows(
            "AT Payment",
            fields=["name", "payment_no", "status", "payment_date", "amount_try", "currency"],
            filters={"policy": offer.get("converted_policy")} if offer.get("converted_policy") else {"name": "not-found"},
            order_by="payment_date desc",
            limit_page_length=5,
        ),
        "renewals": _get_rows(
            "AT Renewal Task",
            fields=["name", "status", "due_date", "priority"],
            filters={"policy": offer.get("converted_policy")} if offer.get("converted_policy") else {"name": "not-found"},
    ...
    files = _get_rows(
        ...
        filters={"attached_to_doctype": "AT Offer", "attached_to_name": offer_name, "is_folder": 0},
        order_by="creation desc",
...
def invalidate_offer_360_cache(offer_name: str):
    if not offer_name:
        return
    cache_key = f"at_offer_360:{offer_name}"
    frappe.cache().delete_value(cache_key)
    LOGGER.debug(f"Invalidated offer 360 cache for: {offer_name}")
...
def _get_related_offers(offer):
    if not offer.customer:
        return []
...
def _get_related_policies(offer):
    if not offer.customer:
        return []
    return frappe.get_list(
        "AT Policy",
...
def _get_activity(offer):
    # This simulates the logic from details_offer.py
    events = []
    ...
    comments = frappe.get_list(
        ...
        filters={"reference_doctype": "AT Offer", "reference_name": offer.name},
        order_by="creation desc",
...
def invalidate_offer_from_doc_event(doc, method=None):
    """Bridge function for hooks to invalidate offer 360 cache."""
    if not doc:
        return
