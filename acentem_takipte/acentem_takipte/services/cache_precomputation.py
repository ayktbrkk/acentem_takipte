...
from acentem_takipte.acentem_takipte.utils.cache_keys import scope_cache_key
...
def precompute_user_scope(user: str | None = None) -> dict:
    ...
    cache_ttl = cint(
        frappe.get_site_config().get(
            "at_scope_cache_ttl_seconds", SCOPE_CACHE_TTL_SECONDS
        )
    )
    ...
    try:
        cache_key = scope_cache_key(user)
        frappe.cache().set_value(cache_key, scope_data, expires_in_sec=cache_ttl)
        frappe.logger().info(
            f"Pre-computed scope for {user}: {len(allowed_branches)} branches, {len(allowed_sales_entities)} sales entities"
        )
    except Exception as e:
...
def get_cached_user_scope(
    user: str | None = None, use_precomputed: bool = True
) -> dict | None:
    """
...
def invalidate_user_scope_cache(user: str | None = None) -> bool:
    ...
    try:
        ...
        frappe.logger().warning(
            f"Failed to invalidate scope cache for {user}: {str(e)}"
        )
        return False
