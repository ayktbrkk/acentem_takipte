...
def _insert(doctype: str, values: dict[str, Any]):
...
def run(
    ...
    for bdef in office_branch_defs:
    ...
    # ══════════════════════════════════════════════════════════════════════
    # 4. Sales Entities -- 5 (hierarchical)
    # ══════════════════════════════════════════════════════════════════════
    root_entity = _upsert_by_field(
        "AT Sales Entity",
        "full_name",
        "Merkez Acente",
        {
    ...
    ekipal = _upsert_by_field(
        ...
        "Ekip Alfa",
        {
            ...
            "commission_share_pct": 30,
            "is_active": 1,
        },
    )
    _inc("sales_entities")

    ekibet = _upsert_by_field(
        "AT Sales Entity",
        "full_name",
        "Ekip Beta",
        {
    ...
    rep_tugce = _upsert_by_field(
        "AT Sales Entity",
        "full_name",
        "Temsilci Tugce",
        {
            "entity_type": "Representative",
            "full_name": "Temsilci Tugce",
            "office_branch": branch_by_code["IST"],
            "parent_entity": ekipal.name,
            "commission_share_pct": 20,
            "is_active": 1,
        },
    )
    _inc("sales_entities")

    rep_emre = _upsert_by_field(
        "AT Sales Entity",
        "full_name",
        "Temsilci Emre",
        {
            "entity_type": "Representative",
            "full_name": "Temsilci Emre",
            "office_branch": branch_by_code["ANK"],
            "parent_entity": ekibet.name,
            "commission_share_pct": 20,
            "is_active": 1,
        },
    )
    ...
    offer_data = [
        ...
        },
        {
            ...
            "status": "Converted",
            "notes": "DASK teklifi policeye donusturuldu",
        },
        {
            "customer": customer_map["cust_1"],
            "sales_entity": rep_tugce.name,
    ...
    for cndata in call_note_data[:seed_count]:
        _insert("AT Call Note", cndata)
        _inc("call_notes")
    if print_output:
    ...
    for acdata in accounting_data[:seed_count]:
        _insert("AT Accounting Entry", acdata)
    ...
    for sdata in segment_data[:seed_count]:
        _insert("AT Segment", sdata)
        _inc("segments")
    if print_output:
    ...
    for cdata in campaign_data[:seed_count]:
        _insert("AT Campaign", cdata)
        _inc("campaigns")
    if print_output:
    ...
    for ddata in document_data[:seed_count]:
        _insert("AT Document", ddata)
        _inc("documents")
    if print_output:
    ...
    reconciliation_data = [
        ...
        },
        {
            ...
            "local_amount_try": 15800,
            "external_amount_try": 15800,
            "resolution_action": "Komisyon duzeltmesi yapildi",
        ...
        {
            "source_doctype": "AT Policy",
            "source_name": policy_map["MPFR-2026-00101"],
            "status": "Ignored",
            "mismatch_type": "Currency",
            "local_amount_try": 3200,
            "external_amount_try": 3205,
            "notes": "Onemsiz fark - yok sayildi",
        },
    ]
    for rdata in reconciliation_data[:seed_count]:
        _insert("AT Reconciliation Item", rdata)
