...
def execute():
    def recalc(sales_entity, commission_amount, fx_rate):
        ...
        while current_entity and current_entity not in visited:
            ...
            if is_root:
                root_entry = {"entity": current_entity, "entity_name": ed.get("full_name") or current_entity, "level": level, "share_pct": pct, "amount": 0.0, "amount_try": 0.0, "status": "Accrued", "office_branch": ob, "is_root": True}
                break
            amt = round(commission * pct / 100, 2)
            non_root_total = round(non_root_total + amt, 2)
            remaining = round(remaining - amt, 2)
            entries.append({"entity": current_entity, "entity_name": ed.get("full_name") or current_entity, "level": level, "share_pct": pct, "amount": round(amt, 2), "amount_try": round(amt * fx, 2), "status": "Accrued", "office_branch": ob, "is_root": False})
            if remaining <= 0.01:
                break
