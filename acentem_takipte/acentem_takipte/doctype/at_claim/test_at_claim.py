        ...
        office_branch_name = frappe.get_doc(
            {
                ...
                "office_branch_name": f"Claim Office {suffix}",
                "office_branch_code": f"COB{suffix[:4]}",
                "city": "Istanbul",
                "is_active": 1,
                "is_head_office": 1,
            }
...
def _random_tax_id() -> str:
    raw = "".join(ch for ch in frappe.generate_hash(length=12) if ch.isdigit())[:9].ljust(9, "1")
    if raw.startswith("0"):
        raw = f"1{raw[1:]}"
