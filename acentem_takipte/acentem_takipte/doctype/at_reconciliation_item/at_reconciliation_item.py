...
class ATReconciliationItem(Document):
    def validate(self):
        ...
        if self.status in ATReconciliationItemStatus.CLOSED:
            if not self.resolved_by:
                self.resolved_by = frappe.session.user
            if not self.resolved_on:
                self.resolved_on = now_datetime()
            if self.status == ATReconciliationItemStatus.IGNORED and not self.resolution_action:
                self.resolution_action = "Ignored"
            if self.status == ATReconciliationItemStatus.RESOLVED and not self.resolution_action:
