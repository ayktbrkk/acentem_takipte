...
def assert_renewal_status_transition(previous_status: str | None, next_status: str | None) -> str:
    ...
    if normalized_next not in allowed:
        frappe.throw(
            _("Invalid renewal status transition: {0} -> {1}").format(normalized_previous, normalized_next)
        )
    return normalized_next


def get_renewal_candidates(*, business_date: date | None = None, lookahead_days: int = 90, limit: int = 2000) -> list[RenewalCandidate]:
...
def build_task_unique_key(*, policy: str, customer: str | None, renewal_date, due_date) -> str:
    renewal_day = getdate(renewal_date) if renewal_date else None
    due_day = getdate(due_date) if due_date else None
    stage = resolve_stage_for_days(date_diff(renewal_day, due_day)) if due_day and renewal_day else None
    if stage:
...
def create_renewal_notification_draft(task_doc) -> str | None:
    ...
    stage = resolve_stage_for_days(date_diff(renewal_date, due_date)) if due_date and renewal_date else None
...
def remediate_stale_renewal_tasks(*, business_date: date | None = None, limit: int = 500) -> dict[str, int]:
    today = getdate(business_date or nowdate())
    rows = frappe.get_all(
        "AT Renewal Task",
        filters={
            "status": ["in", [ATRenewalTaskStatus.OPEN, ATRenewalTaskStatus.IN_PROGRESS, "Completed"]],
            "renewal_date": ["<", today],
            "auto_created": 1,
        },
        fields=["name"],
    ...
    for row in rows:
        ...
        task.notes = f"{existing_notes}\n{remediation_note}".strip() if existing_notes else remediation_note
...
def sync_renewal_outcome(task_doc) -> str | None:
    ...
    lost_reason_code = str(getattr(task_doc, "lost_reason_code", "") or "").strip() or None
    competitor_name = str(getattr(task_doc, "competitor_name", "") or "").strip() or None
