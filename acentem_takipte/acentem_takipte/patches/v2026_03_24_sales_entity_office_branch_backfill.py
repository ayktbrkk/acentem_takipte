...
def execute():
    ...
    # unbounded: all sales entities for office branch backfill, bounded by total entity count - expected max ~10k rows
    rows = frappe.get_all(
        "AT Sales Entity",
    ...
    for row in rows:
        ...
        parent_branch = (
            branch_by_entity.get(parent_name, "").strip() if parent_name else ""
        )
...
def _pick_default_branch() -> str | None:
    ...
    first_any = frappe.db.get_value(
        "AT Office Branch", {}, "name", order_by="creation asc"
    )
