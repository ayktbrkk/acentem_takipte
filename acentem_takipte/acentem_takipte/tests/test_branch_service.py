...
def test_get_user_office_branches_returns_access_rows_for_normal_user():
    ...
    with patch("acentem_takipte.acentem_takipte.services.branches.user_can_access_all_office_branches", return_value=False):
...
def test_get_default_office_branch_prefers_default_flag():
    with patch(
        "acentem_takipte.acentem_takipte.services.branches.get_user_office_branches",
        return_value=[
