...
from acentem_takipte.acentem_takipte.domains.reports.services import scheduled as scheduled_reports
...
def test_save_scheduled_report_config_calls_upsert(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda index=None, config=None: {"index": 2, "config": {"report_key": "payment_status", **(config or {})}},
    )
...
def test_remove_scheduled_report_config_calls_delete(monkeypatch):
    ...
    monkeypatch.setattr(reports, "assert_post_request", lambda *args, **kwargs: None)
    monkeypatch.setattr(reports, "assert_roles", lambda *args, **kwargs: None)
    monkeypatch.setattr(reports, "remove_scheduled_report", lambda index: {"remaining": 1})
...
def test_normalize_scheduled_report_config_coerces_invalid_delivery_channel():
    payload = scheduled_reports.normalize_scheduled_report_config(
        {
            "report_key": "policy_list",
