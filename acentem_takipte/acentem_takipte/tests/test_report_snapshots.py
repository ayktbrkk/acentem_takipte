...
def test_store_report_snapshot_payload_serializes_dates(monkeypatch):
    snapshots: dict[str, SimpleNamespace] = {}
    fixed_today = "2026-03-26"
    ...
    payload = {
        "columns": ["name", "status"],
        "rows": [{"name": "POL-001", "status": "Active", "issue_date": date(2026, 3, 1)}],
        "filters": {"office_branch": "IST"},
    }
    ...
    assert stored["row_count"] == 1
    assert snapshots["SNAP-1"].rows_json == '[{"name": "POL-001", "status": "Active", "issue_date": "2026-03-01"}]'
